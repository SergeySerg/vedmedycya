...
//Route::group(['domain' => getSetting('domain')], function() {	
	...
	Route::get('/{category}', ['middleware' => 'frontend.init', 'uses' => 'Frontend\ArticleController@renderUrl', 'as' => 'article_category'])->where('category', 'скидки|sales|znyzhky|поиск|poshuk|search');
	Route::get('/{lang?}/{category}', ['middleware' => 'frontend.init', 'uses' => 'Frontend\ArticleController@renderUrl', 'as' => 'article_category'])->where('lang', 'ua|ru|en|pl')->where('category', 'скидки|sales|znyzhky|поиск|poshuk|search');
