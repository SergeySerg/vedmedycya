...
class AdminArticlesController extends Controller
{
    ...
    public function index(Request $request, $type)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->has('id')) {
            ...
            } elseif ($request->has('base_hotel_id')) {
                ...
                if ($base_hotel_id) {
                        ...
                        $articles = $slides->filter(function ($admin_article) use ($base_hotel_id, $season) {
                            //dd($admin_article);

                            if ($admin_article->article_parent()->first()) {
                                $admin_article['parent_hotel'] = $admin_article->article_parent()->first()->title;
                                $admin_article = $admin_article->article_parent()->first()->id == $base_hotel_id;
                            } else {
                                $admin_article['parent_hotel'] = "Brand Page";
                                $admin_article = $admin_article->article_id == $base_hotel_id;
                ...
                if ($base_hotel_id == 'all') {
                    //dd('we');
                    session([
                        'base_hotel_id' => $base_hotel_id,
                        'season' => $season
                    ]);
                    $articles = $slides->map(function ($admin_article) use ($season) {
